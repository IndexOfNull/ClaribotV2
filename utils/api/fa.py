...
class FurAffinity(FABaseObject):
	...
	async def get_front_page(self,**kwargs):
		try:
			...
			response = await self.http.get('http://www.furaffinity.net')
	...
	async def search(self,query,**kwargs):
		try:
			...
			for i in types:
				if i in type:
					data["type-"+i] = "on"
			response = await self.http.post(self.base_url+'search',params=data,headers=headers)
			if response:
				data = BeautifulSoup(response,"html.parser")
		...
		try:
			...
			if response:
				...
				if sysmsg:
					...
					return Post(title=title,id=id,content_url=content_url,preview_base=prev_template,source=url,keywords=keywords,by=by)
			else:
				return
		except Exception as e:
			print(e)
